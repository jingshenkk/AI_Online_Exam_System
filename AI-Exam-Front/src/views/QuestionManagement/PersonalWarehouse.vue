    ...
    <div class="common-module-query-box">
      ...
      <div class="module-query-item">
        ...
        <el-select v-model="queryInfo.type" placeholder="请选择题目类型" style="width: 240px" clearable>
          ...
          <el-option key="3" label="简答题" value="essay"/>
        </el-select>
      </div>
      <div class="module-query-item">
      ...
      >
        ...
        <el-table-column prop="type" label="试题类型" align="center" width="120">
          <template #default="scope">
            ...
            <span v-else-if="scope['row']['type'] === 'essay'">简答题</span>
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操 作" align="center" width="280" :resizable="false">
          <template #default="scope">
            ...
            <template v-if="scope['row']['type'] === 'essay'">
              <el-divider direction="vertical"/>
              <el-button
                  link
                  size="small"
                  type="primary"
                  :icon="ClipboardList"
                  @click="handleGoScoringPoints(scope['row']['id'])"
              >
                细则
              </el-button>
            </template>
    ...
    >
      <el-form :model="formData" ref="formRef">
        ...
        <el-form-item label="参考答案" :label-width="formLabelWidth" prop="answer" required>
          ...
          <el-input v-else type="textarea" :rows="4" v-model="formData.answer" placeholder="请输入标准答案/参考答案" clearable/>
        </el-form-item>
      ...
      <div class="question-detail-box">
        <div class="detail-tag-box detail-common">
          <el-tag style="margin-right: 10px" :icon="Bookmark">
            <el-icon><Tag /></el-icon>
            {{ detailData['type'] === 'select' ? '选择题' : detailData['type'] === 'judge' ? '判断题' : '简答题' }}
          </el-tag>
          <el-tag style="margin-right: 10px" :icon="Bookmark">
            <el-icon><Tag /></el-icon>
        ...
        <div class="detail-common detail-answer-box">
          ...
          <span v-else-if="detailData['type'] === 'judge'" style="display: flex;align-items: center;">
            <Check v-if="detailData['answer'] == 'T'" style="height: 16px"/>
            <X v-else style="height: 15px"/>
          </span>
          <div v-else style="margin-top: 10px;white-space: pre-wrap;">{{ detailData['answer'] }}</div>
        </div>
...
import {
  Ban, BookHeart, Check, ClipboardList, Info, Plus, Search, Minus,
...
watch(() => formData.value.type, (newQuestion) => {
  if (newQuestion !== 'judge' && optType.value === 'C') {
...
// 跳转到评分细则管理页面
const handleGoScoringPoints = (questionId: string) => {
  router.push(`/scoringPoints/${questionId}`)
}
