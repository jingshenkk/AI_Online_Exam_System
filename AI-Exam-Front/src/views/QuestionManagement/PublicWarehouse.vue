    ...
    >
      <div class="question-detail-box">
        ...
        <div class="detail-common detail-answer-box">
          ...
          <span v-if="detailData['type'] === 'select' || detailData['type'] === 'essay'">{{ detailData['answer'] }}</span>
...
import {BookCheck, Bookmark, Check, ClipboardList, Heart, HeartOff, Info, Search, Tag, X} from "lucide-vue-next";
...
// 跳转到评分细则管理页面（公共题库来源，只读模式）
const handleGoScoringPoints = (questionId: string) => {
  router.push(`/scoringPoints/${questionId}?source=public`)
}
</script>
