<template>
  <div class="exam-result-detail-main">
    ...
    <common-result-view :paper-info="paperInfo" :exam-result-answers="examResultAnswers" :is-collect="false" :is-teacher="true" @updateData="refreshData"/>
  </div>
</template>
...
// 处理页面返回
const goBack = () => {
  const sourceUrl = localStorage.getItem('RESULT_DETAILS_SOURCE_URL_TEACHER')
...
onBeforeUnmount(() => {
  // 销毁
  localStorage.removeItem('RESULT_DETAILS_SOURCE_URL_TEACHER')
})
</script>
