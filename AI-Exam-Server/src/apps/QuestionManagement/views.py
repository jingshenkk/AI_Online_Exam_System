...
from .models import Questions, QuestionsFavorite, ScoringPoint
from .serializers import QuestionSerializer, QuestionFavoriteSerializer
from .serializers import ScoringPointSerializer
...
class RubricGenerateView(APIView):
    """LLM辅助生成评分细则"""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, **kwargs):
        ...
        if question.type != 'essay':
            return api_response(ResponseCode.BAD_REQUEST, '仅主观题支持生成评分细则')
        
        if not question.answer:
            return api_response(ResponseCode.BAD_REQUEST, '题目缺少标准答案，无法生成评分细则')
        
        # 获取该题目在试卷中的分值（从请求参数获取）
        ...
        try:
            ...
            # 先删除该题目已有的未审核评分细则
            ScoringPoint.objects.filter(question_id=question_id, is_approved=False).delete()
            
            # 保存新生成的评分细则
            created_points = []
            for idx, point_data in enumerate(rubric_data['points']):
                point = ScoringPoint.objects.create(
            ...
            serializer = ScoringPointSerializer(created_points, many=True)
            return api_response(ResponseCode.CREATED, '评分细则生成成功', serializer.data)
            
        except Exception as error:
            logger.error(f"评分细则生成失败: {error}")
            return api_response(ResponseCode.INTERNAL_SERVER_ERROR, f'评分细则生成失败: {str(error)}')

class ScoringPointView(APIView):
    """评分细则管理"""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, **kwargs):
        ...
        if question_id is None:
            return api_response(ResponseCode.BAD_REQUEST, '缺少题目ID')
        points = ScoringPoint.objects.filter(question_id=question_id).order_by('sort_order')
        serializer = ScoringPointSerializer(points, many=True)
        return api_response(ResponseCode.SUCCESS, '查询成功', serializer.data)
    
    def post(self, request, **kwargs):
        """创建评分细则"""
        serializer = ScoringPointSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            data = Response(serializer.data)
            return api_response(ResponseCode.SUCCESS, '创建成功', data.data)
...
class ScoringPointDetailView(APIView):
    """单个评分细则操作"""
    permission_classes = [IsAuthenticated]
    ...
    def put(self, request, **kwargs):
        """修改评分细则"""
        try:
            point = ScoringPoint.objects.get(id=kwargs['id'])
        except ScoringPoint.DoesNotExist:
            return api_response(ResponseCode.NOT_FOUND, '评分细则不存在')
        serializer = ScoringPointSerializer(point, request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            data = Response(serializer.data)
            return api_response(ResponseCode.SUCCESS, '修改成功', data.data)
...
class ScoringPointApproveView(APIView):
    """审核评分细则"""
    permission_classes = [IsAuthenticated]
    ...
    def put(self, _, **kwargs):
        ...
        point.save()
        serializer = ScoringPointSerializer(point)
        data = Response(serializer.data)
        return api_response(ResponseCode.SUCCESS, '审核通过', data.data)

class ScoringPointBatchApproveView(APIView):
    """批量审核评分细则"""
    permission_classes = [IsAuthenticated]
    ...
    def put(self, request):
        ...
        if not point_ids:
            return api_response(ResponseCode.BAD_REQUEST, '缺少评分细则ID列表')
        updated_count = ScoringPoint.objects.filter(id__in=point_ids).update(is_approved=True)
        return api_response(ResponseCode.SUCCESS, f'已审核通过 {updated_count} 条评分细则')
